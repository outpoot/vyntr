<script lang="ts">
    ...
    import DetailsPanelSpellingMessage from '$lib/components/self/DetailsPanelSpellingMessage.svelte';
    ...
    import HeartCrack from "lucide-svelte/icons/heart-crack.svelte";
    ...
    <div class="mt-4 flex flex-col items-start gap-4">
        ...
        <div class="flex w-full flex-row gap-4">
            <div class="w-full min-w-0 flex-1 lg:max-w-[700px]">
                {#if !isLoading}
                    {#if searchData.correction}
                        <div
                                class="overflow-hidden rounded-xl py-2 text-muted"
                        >
                            Showing partial results of query <a
                                href="/search?q={encodeURIComponent(searchData.correction.newQuery)}"
                                onclick={() => query = searchData.correction?.newQuery || query}
                        >{searchData.correction.newQuery}</a>
                        </div>

                        {#if searchData.correction.shownMessage}
                            <div class="mb-6">
                                <DetailsPanelSpellingMessage detail={searchData.correction.shownMessage}/>
                            </div>
                        {/if}
                    {/if}
